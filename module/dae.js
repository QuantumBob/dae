    ...
    static createValidMods(characterSpec = game.system.model.Actor.character) {
        ...
        if (["dnd5e", "sw5e"].includes(game.system.id)) {
            var specials = {
                ...
                "macro.execute.local": ["", ACTIVE_EFFECT_MODES.CUSTOM],
                "macro.execute.GM": ["", ACTIVE_EFFECT_MODES.CUSTOM],
                "macro.itemMacro": ["", ACTIVE_EFFECT_MODES.CUSTOM],
                "macro.itemMacro.local": ["", ACTIVE_EFFECT_MODES.CUSTOM],
                "macro.itemMacro.GM": ["", ACTIVE_EFFECT_MODES.CUSTOM],
                ...
                "data.attributes.ac.EC": [0, -1],
                "data.attributes.ac.AR": [0, -1]
            };
...
function applyDaeEffects(specList, completedSpecs, allowAllSpecs) {
    ...
    for (let c of changes) {
        ...
        if (typeof ValidSpec.allSpecsObj[c.key]?.sampleValue === "number" && typeof currentValue !== "number") {
...
function prepareData(wrapped) {
    ...
    const disAdvSte = hasHeavy || getProperty(this.data, "flags.midi-qol.disadvantage.skill.ste");
    setProperty(this.data, "flags.midi-qol.disadvantage.skill.ste", hasHeavy);
...
function _onCreateActiveEffect(...args) {
    ...
    if (token && effect.data.changes && !effect.disabled && !effect.isSuppressed) {
        let changeLoop = async () => {
            try {
                const effectData = await daeMacro("on", parent, effect.data, {});
                for (let change of effect.data.changes) {
                    if (cubActive && change.key === "macro.CUB") {
                        await game.cub.addCondition(change.value, [token]);
                    }
                    if (ceActive && change.key === "macro.CE") {
                        if (isNewerVersion("2.0.0", game.modules.get("dfreds-convenient-effects")?.data.version)) {
                            //@ts-ignore
                    ...
                    if (tokenMagic && change.key === "macro.tokenMagic")
                        await addTokenMagicChange(parent, change, tokens, tokenMagic); //TODO check disabled
                }
            }
            catch (err) {
...
async function _preCreateActiveEffect(wrapped, ...args) {
    ...
    try {
        ...
        if (this.data.flags?.dae?.durationExpression) {
                    ...
                    for (let change of effect.data.changes) {
                        ...
                        if (ceActive && change.key === "macro.CE") {
                            if (isNewerVersion("2.0.0", game.modules.get("dfreds-convenient-effects")?.data.version)) {
                                ...
                                await game.dfreds.effectInterface.removeEffect({ effectName: change.value, uuid: parent.uuid });
                            }
                        }
                        if (CV)
                            await removeCVChange(parent, change, [token], CV);
                        if (tokenMagic && change.key === "macro.tokenMagic")
                            await removeTokenMagicChange(parent, change, tokens, tokenMagic);
                    }
                }
                else {
                    for (let change of effect.data.changes) {
                        ...
                        if (CV)
                            await addCVChange(parent, change, [token], CV);
                        if (tokenMagic && change.key === "macro.tokenMagic")
                            await addTokenMagicChange(parent, change, tokens, tokenMagic);
                    }
                }
            }
        }
    ...
    let changesLoop = async () => {
        try {
            ...
            if (token && effect.data.changes) {
                for (let change of effect.data.changes) {
                    if (tokenMagic && change.key === "macro.tokenMagic")
                        await removeTokenMagicChange(actor, change, tokens, tokenMagic);
                    if (CV)
                    ...
                    if (ceActive && change.key === "macro.CE") {
                        if (isNewerVersion("2.0.0", game.modules.get("dfreds-convenient-effects")?.data.version)) {
                            ...
                            await game.dfreds.effectInterface.removeEffect({ effectName: change.value, uuid: token.actor.uuid });
                        }
                    }
                    if (cubActive && change.key === "macro.CUB") {
                        await game.cub.removeCondition(change.value, [token], { warn: false });
                ...
                const timesup = options ? options["times-up"] : undefined;
                await daeMacro("off", actor, effect.data, { "times-up": timesup });
                if (entityToDelete)
 ...
 */
export async function doEffects(item, activate, targets = undefined, { whisper = false, spellLevel = 0, damageTotal = null, itemCardId = null, critical = false, fumble = false, effectsToApply = [], removeMatchLabel = false }) {
    return await applyNonTransferEffects.bind(item)(activate, targets, { effectsToApply, whisper, spellLevel, damageTotal, itemCardId, critical, fumble, removeMatchLabel });
}
...
export async function applyNonTransferEffects(activate, targets, { whisper = false, spellLevel = 0, damageTotal = null, itemCardId = null, critical = false, fumble = false, tokenId: tokenId, effectsToApply = [], removeMatchLabel = false }) {
    if (!targets)
        return;
    ...
    if (gmTargets.length > 0)
        await socketlibSocket.executeAsGM("applyActiveEffects", { userId: game.user.id, activate, activeEffects: appliedEffects, targets: gmTargets, itemDuration: this.data.data.duration, itemCardId, removeMatchLabel });
    // requestGMAction(GMAction.actions.applyActiveEffects, { activate, activeEffects: appliedEffects, targets: gmTargets, itemDuration: this.data.data.duration, itemCardId })
    if (localTargets.length > 0) {
        const result = await applyActiveEffects(activate, localTargets, appliedEffects, this.data.data.duration, itemCardId, removeMatchLabel);
    }
}
