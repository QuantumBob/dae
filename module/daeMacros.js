...
export let teleport = async (token, targetScene, xpos, ypos) => {
    ...
    Hooks.once("canvasReady", async () => {
        await socketlibSocket.executeAsGM("createToken", { userId: game.user.id, startSceneId: sourceSceneId, targetSceneId: targetScene.id, tokenData: token.data.toObjct(), x: xpos, y: ypos });
...
export async function macroActorUpdate(...args) {
    // add effect variable to get stack type ## USE lastArg
    ...
    if (["on", "each"].includes(args[0])) {
        ...
        switch (type) {
            ...
            case "number":
                ...
                const op = value[0];
                if (["+", "-", "*", "/"].includes(op))
                    value = (await new Roll(`${actorValue}${value}`, rollContext).roll()).total;
                else
                    value = (await new Roll(value, rollContext).roll()).total;
                break;
            default: // assume a string
        }
        const update = {};
        // need to use a stack to pop old values back
        let ov = [];
        // stack type = 
        // none - oldValue can = actorValue as it should be first and only value
        // multi - each value can be removed individually so we need array of values
        if (lastArg.efData.flags.dae.stackable === "multi") {
            if (hasProperty(actor.data, fieldDef + '.oldValue')) {
                ov = getProperty(actor.data, fieldDef + '.oldValue');
            }
            ov.push(actorValue);
        } else { // stackable is count or none
            if (hasProperty(actor.data, fieldDef + '.oldValue')) {
                ov[0] = getProperty(actor.data, fieldDef + '.oldValue');
            } else {
                ov[0] = actorValue;
            }
        }
        update[fieldDef] = { oldValue: ov, updateValue: value };
        ...
        if (typeof undo === "string" && undo === "remove") {
            restoreValue = Math.max(0, actorValue - (updateValue - oldValue.pop()));
        }
            ...
            restoreValue = (await new Roll(`${undo}`, rollContext).roll()).total;
        }
        ...
        if (undo === true)
            if (lastArg.efData.flags.dae.stackable === "count") {
                restoreValue = oldValue[0] ?? 1;
            } else if (lastArg.efData.flags.dae.stackable === "multi") {
                restoreValue = oldValue.pop();

            }
            else if (lastArg.efData.flags.dae.stackable === "none") {
                // restoreValue = oldValue.pop();
                restoreValue = oldValue[0] ?? 1;
            }
        if (undo !== false) {
            ...
            update[fieldDef] = { oldValue: oldValue, updateValue: restoreValue };
            await actor.update(update);
